...
class MyTabView(customtkinter.CTkTabview):
    def __init__(self, master, **kwargs):
        ...
        self.label1 = customtkinter.CTkLabel(master=self.tab(
            "Home"), corner_radius=10, width=900, height=900)  # textvariable=text_var,
        self.label1.grid(row=0, column=0, padx=20, pady=20)

# width=900, height=900
        self.label2 = customtkinter.CTkLabel(master=self.tab(
            "Generer un mot de passe"), textvariable=text_var, width=900, height=900)
        self.label2.grid(row=0, column=0, padx=20, pady=20)

        self.label3 = customtkinter.CTkLabel(master=self.tab(
            "Mes mots de passes"), textvariable=text_var, width=900, height=900)
        ...
        self.button = customtkinter.CTkButton(master=self.tab(
            "Generer un mot de passe"), text="Generer", command=self.generate_password)  # command=generate_password)
